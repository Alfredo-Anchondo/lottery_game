...
def activity_of_day
  ...
  @quiniela_ticket = QuinielaUser.where("purchase_date::date = ?",@date_param.to_date)
	...
	def send_mails_all
		...
		User.send_mails_all(params[:mails], params[:content], params[:subject])
		render json: true
	end
  ...
  private
    ...
    def user_params
      params.require(:user).permit(:name, :provider, :tokens,  :last_name, :address_1, :address_2, :zip_code, :age, :email, :phone, :cellphone, :balance, :role_id, :country, :state, :city, :int_number, :photo, :ext_number, :username, :password, :openpay_id, :favorite_team, :birthday, :gender, :language, :friend_reference, :gift_credit, :reference_by_friend)
    end
end
